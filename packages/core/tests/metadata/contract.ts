/**
 * Contract tests for the `MetadataStore` interface.
 *
 * The same test suite is run against every concrete adapter
 * (memory, SQLite, Postgres). The adapter factory is the only
 * thing that differs — the contract behavior is uniform.
 *
 * Usage:
 *   import { runMetadataStoreContract } from "./contract";
 *   runMetadataStoreContract("memory", () => createMemoryStore());
 *   runMetadataStoreContract("sqlite-memory", () => createSqliteStore(new Database(":memory:")));
 */
import { describe, it, expect, beforeEach } from "vitest";
import { asTenantId, asUserId } from "@/types/branded";
import { FileSystemError } from "@/errors";
import type { MetadataStore, FileNode } from "@/metadata";
...
export const runMetadataStoreContract = (
  name: string,
  ...
  describe(`MetadataStore contract: ${name}`, () => {
    ...
    describe("moveNode", () => {
      ...
      it("renames a node in place when newParentId is null and newName is given", async () => {
    ...
    // -----------------------------------------------------------------
    describe("reconcile", () => {
      it("returns a no-op result with a non-negative scanned count", async () => {
